...
CREATE INDEX idx_health_maps_golf_club ON health_map_tilesets(golf_club_id);
CREATE INDEX idx_health_maps_source ON health_map_tilesets(source_tileset_id);
CREATE INDEX idx_health_maps_active ON health_map_tilesets(is_active) WHERE is_active = true;

-- Enable RLS
ALTER TABLE health_map_tilesets ENABLE ROW LEVEL SECURITY;

-- RLS Policies (same as golf_course_tilesets)
