...
serve(async (req) => {
  ...
  try {
    ...
    // Derive allowed course prefixes from the user's club name if available
    let allowedPrefixes: string[] = [basePrefix];
    if (me.club_id) {
    ...
    switch(body.action) {
      ...
      case 'getGetUrl': {
        ...
        if (me.role !== 'admin') {
          const permitted = allowedPrefixes.some((p) => key.startsWith(p));
          if (!permitted) return new Response(JSON.stringify({ error: 'Forbidden' }), { headers: { ...corsHeaders, 'Content-Type': 'application/json' }, status: 403 });
        }
        const url = await createAWS4Url(body.action === 'getPutUrl' ? 'PUT' : 'GET', bucket, accountId, key, accessKeyId, secretAccessKey, region, expiresIn, body.action === 'getPutUrl' ? 'UNSIGNED-PAYLOAD' : '');
        return new Response(JSON.stringify({ url, key, method: body.action === 'getPutUrl' ? 'PUT' : 'GET' }), { headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
      }
