...
serve(async (req) => {
  ...
  try {
    ...
    switch (action) {
      case 'assign': {
        const { clientId, golfClubId }: AssignCourseRequest = await req.json()

        if (!clientId || !golfClubId) {
          return new Response(
            JSON.stringify({ error: 'Missing clientId or golfClubId' }),
      ...
      case 'remove': {
        const { clientId, golfClubId }: RemoveCourseRequest = await req.json()
