...
serve(async (req) => {
  ...
  try {
    ...
    // Get layers for this golf club (note: golf_club_id, not golf_course_id)
    const { data: layers, error } = await supabaseClient
      .from('vector_layers')
      .select('*')
      .eq('golf_club_id', golfCourseId)
      .eq('is_active', true)
    ...
    const layersWithUrls = layers.map((layer) => {
      ...
      const publicUrl = r2PublicUrl 
        ...
        : `https://${Deno.env.get('CLOUDFLARE_R2_ACCOUNT_ID')}.r2.cloudflarestorage.com/${Deno.env.get('CLOUDFLARE_R2_BUCKET_NAME') || 'map-stats-tiles-prod'}/${layer.r2_key}`
