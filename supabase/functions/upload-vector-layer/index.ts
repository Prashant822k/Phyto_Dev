...
serve(async (req) => {
  ...
  try {
    ...
    const golfCourseId = formData.get('golf_club_id') as string
    ...
    if (!file || !golfCourseId || !courseName || !name) {
      ...
      return new Response(
        JSON.stringify({ error: 'Missing required fields: file, golf_club_id, course_name, and name are required' }),
    ...
    const r2BucketName = Deno.env.get('CLOUDFLARE_R2_BUCKET_NAME') ?? 'map-stats-tiles-prod'
