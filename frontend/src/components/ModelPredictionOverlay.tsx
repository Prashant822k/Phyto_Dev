...
export default function ModelPredictionOverlay({
  ...
  const loadPredictions = async () => {
    if (!golfClubId) {
      console.log('[ModelPredictionOverlay] No golfClubId provided');
      return;
    }
    
    console.log('[ModelPredictionOverlay] Loading predictions for:', golfClubId);
    setLoading(true);
    try {
      const list = await modelInferenceService.listPredictions(golfClubId);
    ...
    if (isVisible) {
      ...
      if (!geojson) {
        try {
          ...
          geojson = await modelInferenceService.getPrediction(golfClubId, predictionId);
  ...
  const deletePrediction = async (predictionId: string) => {
    ...
    try {
      ...
      await modelInferenceService.deletePrediction(golfClubId, predictionId);
  ...
  const createTestPrediction = async () => {
    ...
    try {
      ...
      const result = await modelInferenceService.storePrediction(golfClubId, testGeojson);
