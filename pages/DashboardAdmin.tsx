...
const DashboardAdmin = () => {
  ...
  const [tilesets, setTilesets] = useState<Array<{ r2_folder_path: string, golf_club_id: string }>>([])
  const [newFolder, setNewFolder] = useState('')
  // GeoJSON overlay upload state
  const [overlayFiles, setOverlayFiles] = useState<File[]>([])
  const [isUploadingOverlays, setIsUploadingOverlays] = useState(false)
  const [overlayUploadProgress, setOverlayUploadProgress] = useState(0)
  const [overlayCurrentStep, setOverlayCurrentStep] = useState('')
  const [clubOverlays, setClubOverlays] = useState<string[]>([])
  const [overlayMap, setOverlayMap] = useState<Record<string, string>>({})
  const [newCatId, setNewCatId] = useState('')
  const [newCatKey, setNewCatKey] = useState('')
  const [overlayJsonText, setOverlayJsonText] = useState('')
  ...
  // Normalize a course folder name: remove trailing '/tiles', trim slashes/spaces
  const normalizeFolder = (s: string) => {
    const base = (s || '').trim();
    const noTiles = base.replace(/\/+tiles\/?$/i, '');
    return noTiles.replace(/^\/+|\/+$/g, '');
  };
  ...
  // Load tilesets for folder selection
  useEffect(() => {
    (async () => {
      try {
        const { data, error } = await supabase
          .from('golf_course_tilesets')
          .select('r2_folder_path, golf_club_id')
        if (!error && data) setTilesets(data)
      } catch {}
    })()
  }, [])
  
  // Auto-refresh files list when course/club selection changes
  useEffect(() => {
    (async () => {
      if (!selectedClub || !courseFolder) {
        setClubOverlays([])
        setOverlayMap({})
        return
      }
      try {
        const folder = normalizeFolder(courseFolder)
        const prefix = `${folder}/tiles/`
        const { items } = await R2Service.list(prefix)
        setClubOverlays(items.map(i => i.key || '').filter(Boolean))

        // Load existing overlays mapping (Option B) for this club+folder
        const { data: existing } = await supabase
          .from('golf_course_tilesets')
          .select('overlays, r2_folder_path')
          .eq('golf_club_id', selectedClub)
          .eq('r2_folder_path', `${folder}/tiles`)
          .maybeSingle()
        const initial = (existing?.overlays as any) || {}
        setOverlayMap(initial)
      } catch (e) {
        toast({
      ...
      <Tabs defaultValue="tiles" className="w-full">
        ...
        <TabsContent value="upload">
          <Card>
            ...
            <CardContent>
              <div className="text-sm text-muted-foreground">This section has been removed.</div>
            </CardContent>
          </Card>
        </TabsContent>
